...
func (e *endpoint) HandlePacket(r *stack.Route, pkt *stack.PacketBuffer) {
	...
	for firstHeader := true; ; firstHeader = false {
		...
		case header.IPv6FragmentExtHdr:
			...
			// If the length and offset of a fragment are such that the payload length of
			// the packet reassembled from the fragment would exceed 65,535 octets, it
			// must be discarded, as per RFC 8200 section 4.5.
			if int(start)+fragmentPayloadLen > 65535 {
				r.Stats().IP.MalformedPacketsReceived.Increment()
				r.Stats().IP.MalformedFragmentsReceived.Increment()
				return
			}
